...
def dfs_heuristic_included(mystatedict,statedict,numcolors,currentstate,states,num,legal_colors):
    global backtracks
    all_states.append(mystatedict.copy())
    for i in range(len(currentstate.next)):
        temp_legal_colors = legal_colors.copy()
        mystatedict[currentstate.next[0].myname] = currentstate.next[i].mycolor   

        if mystatedict.get(currentstate.next[0].myname) in getcolors(statedict[currentstate.next[0].myname],mystatedict):
            #print("continued")
            continue 
        if num == len(states) - 1:
        ...
        ans = dfs_heuristic_included(mystatedict,statedict,numcolors,currentstate.next[i],states,num+1,temp_legal_colors)
        if ans[0] == 1:
            return 1,mystatedict
