...
def init_colors(n):
    for i in range(n):
        listofcolors.append(random.randint(0,255))

def random_color(n):
    return tuple(listofcolors)
...
def generatecols(states,i,number_of_colors,colors):
    tlist = []
...
def forward_checking(mystatedict,statedict,number_of_colors,current_state,states,num):
    ...
    for i in range(len(current_state.next)):
        ...
        temp_listofcolors = listofcolors.copy()
        remove_colors = getcolors(states[num+1],mystatedict)
        temp_listofcolors = [x for x in temp_listofcolors if x not in remove_colors]
        current_state.next[i].next = generatecols(states,num+1,number_of_colors,temp_listofcolors)
   

        ans = forward_checking(mystatedict,statedict,number_of_colors,current_state.next[i],states,num+1)
        if ans[0] == 1:
            return 1,mystatedict
...
def init(states,statedict,number_of_colors):
    colors = listofcolors
...
"Alaska":[]
    ...
    root = init(states,statedict,number_of_colors)

    start_time = time.time()
    answer = forward_checking(mystatedict,statedict,number_of_colors,root,states,0)
    end_time = time.time()
    count = 0 
    for key in answer[1]:
        count+=1
